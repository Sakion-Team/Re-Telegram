...
public class NEWAntiRecall {
    ...
    public static void markMessagesDeleted(Object thisMessagesStorage, final long dialogId, ArrayList<Integer> delMsg) {
        ...
        storage.post(() -> {
            ...
            Object cursor = XposedHelpers.callMethod(db, AutomationResolver.resolve("SQLiteDatabase", "queryFinalized", AutomationResolver.ResolverType.Method), String.class, Object[].class, query, new Object[]{ });
            Object state = XposedHelpers.callMethod(db, AutomationResolver.resolve("SQLiteDatabase", "executeFast", AutomationResolver.ResolverType.Method), String.class, update);
            ...
            while ((boolean) XposedHelpers.callMethod(cursor, AutomationResolver.resolve("SQLiteCursor", "next", AutomationResolver.ResolverType.Method))) {
                Object data = XposedHelpers.callMethod(cursor, AutomationResolver.resolve("SQLiteCursor", "byteBufferValue", AutomationResolver.ResolverType.Method), int.class, 0);
                int mid = (int) XposedHelpers.callMethod(cursor, AutomationResolver.resolve("SQLiteCursor", "intValue", AutomationResolver.ResolverType.Method), int.class, 1);
                long lastDialogId = (long) XposedHelpers.callMethod(cursor, AutomationResolver.resolve("SQLiteCursor", "longValue", AutomationResolver.ResolverType.Method), int.class, 2);

                XposedHelpers.callMethod(data, "position", int.class, 4);
                int flags = (int)XposedHelpers.callMethod(data, "readInt32", boolean.class, true);
                flags |= FLAG_DELETED;
                XposedHelpers.callMethod(data, "position", int.class, 4);
                XposedHelpers.callMethod(data, "writeInt32", int.class, flags);
                XposedHelpers.callMethod(data, "position", int.class, 0);
                ...
                XposedHelpers.callMethod(state, AutomationResolver.resolve("SQLitePreparedStatement", "bindByteBuffer", AutomationResolver.ResolverType.Method), int.class, data.getClass(), 1, data);
                XposedHelpers.callMethod(state, AutomationResolver.resolve("SQLitePreparedStatement", "bindLong", AutomationResolver.ResolverType.Method), int.class, long.class, 2, lastDialogId);
                XposedHelpers.callMethod(state, AutomationResolver.resolve("SQLitePreparedStatement", "bindInteger", AutomationResolver.ResolverType.Method), int.class, int.class, 3, mid);
    ...
    public static void initProcessing(ClassLoader classLoader) {
            ...
            XposedBridge.hookMethod(markMessagesAsDeletedMethod, new AbstractMethodHook() {
                ...
                protected void beforeMethod(XC_MethodHook.MethodHookParam param) {
                    if (Configs.isAntiRecall() && param.args[1] instanceof ArrayList) {
                        ...
                        XposedHelpers.callMethod(param.thisObject, markMessagesAsDeletedMethod.getName(), )
        ...
        for (Method updateDialogsWithDeletedMessagesMethod : updateDialogsWithDeletedMessagesMethods) {
            XposedBridge.hookMethod(updateDialogsWithDeletedMessagesMethod, new AbstractMethodHook() {
                @Override
                protected void beforeMethod(MethodHookParam param) {
                    if (Configs.isAntiRecall() && param.args[2] instanceof ArrayList) {
        ...
        XposedHelpers.findAndHookMethod(notificationCenter, AutomationResolver.resolve("NotificationCenter", "postNotificationName", AutomationResolver.ResolverType.Method), int.class, Object[].class, new AbstractMethodHook() {
            ...
            protected void beforeMethod(MethodHookParam param) {
                ...
                if (Configs.isAntiRecall() && id == messagesDeletedValue) {
                    ...
                    param.setResult(null);
                    //markMessagesDeleted(messagesController, dialogID, arrayList);
                }
            }
        });
